$(document).ready(function () {
    ...
    var cargarDatos = function () {
        $.getJSON("/api/Actividades", function (data) {
            console.log(data);
            var filas = '';
            data.forEach(function (fila) {
                filas += '<tr><td>' + fila['Codigo_Consecutivo'] + '</td><td>' + fila['Prefijo'] +
                    '</td><td>' + fila['Nombre'] + '</td><td>' + fila['Descripcion'] + '</td><td><img id="imagen" class="img-fluid" height="200px" width="200px" src="' + fila["Imagen"] + '" />' +
                    '</td><td><a href=' + 'actividades.html?codigo=' + fila['Codigo_Consecutivo'] + '>' + "Editar" + '</a>' + '</td></tr>';
            })
